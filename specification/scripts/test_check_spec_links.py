...
PROTO = 'xrCreateInstance'
STRUCT = 'XrInstanceCreateInfo'
EXT = 'XR_KHR_opengl_enable'
...
def test_missing_macro(ckr):
    ...
    assert not ckr.check(f'{PROTO}>>').messages
    assert not ckr.check(f'{PROTO} asdf>> table').messages
...
def test_should_entity_be_text():
    ...
    assert not shouldEntityBeText('API_ENUM_i_EXT', None)
...
def test_extension(ckr):
    ...
    assert (
        loneMsgReplacement(ckr.check(f'asdf {EXT} asdf'))
    ...
    assert (
        loneMsgReplacement(ckr.check(f'asdf {EXT.upper()} asdf'))
        == expected_replacement
    )

    # Extension name using wrong/old macro: ename isn't for extensions.
    assert (
        loneMsgReplacement(ckr.check(f'asdf ename:{EXT} asdf'))
        == expected_replacement
    )

    # Extension name using wrong macro: elink isn't for extensions.
    assert (
        loneMsgReplacement(ckr.check(f'asdf elink:{EXT} asdf'))
        == expected_replacement
    )
    ...
    # case fixed
    assert (
        loneMsgReplacement(ckr.check(f'asdf elink:{EXT.upper()} asdf'))
        == expected_replacement
    )

    # This shouldn't cause errors because this is how we want it to look.
    assert not ckr.check(f'asdf apiext:{EXT} asdf').messages
...
def test_refpage_name(ckr):
    ...
    assert (
        ckr.check(f"[open,refpage='{PROTO}',desc='',type='']").numDiagnostics()
        == 0
    )

    # Should error: does not exist.
    assert ckr.check("[open,refpage='bogus',desc='',type='']").numDiagnostics() == 1
...
def test_refpage_missing_desc(ckr):
    ...
    assert ckr.check("[open,refpage='',desc='',type='']").numDiagnostics() == 1
...
def test_refpage_type(ckr):
    ...
    assert not ckr.check(
        f"[open,refpage='{PROTO}',desc='',type='protos']"
    ).messages

    # Should error: this is of type 'protos', not 'structs'.
    assert ckr.check(f"[open,refpage='{PROTO}',desc='',type='structs']").messages
...
def test_refpage_xrefs(ckr):
    ...
    assert not ckr.check(
        f"[open,refpage='',desc='',type='protos',xrefs='{STRUCT}']"
    ).messages
    ...
    assert (
        loneMsgReplacement(
            ckr.check(f"[open,refpage='',xrefs='{STRUCT.lower()}']")
        )
        == STRUCT
    )
